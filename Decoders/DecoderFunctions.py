from sqlalchemy import create_engine, Column, Integer, String, Boolean, ForeignKey, DateTime,Table, Text
...
def getusers(useruuid=None, role=None):
    ...
    for user in all_users:
        user_dict = {
            ...
            'Role': user.Role,
            'DataSources': [{'UUID': datasource.DataSourceUUID,'Name': datasource.data_source.Name} for datasource in user.data_sources]
        }
...
def delete_dataset(dataset_uuid):
    ...
    try:
        ...
        for file in dataset.files:
            ...
            success, message = remove_file_from_dataset(dataset_uuid,file.Filename)   
            print(success,message) 
            #session.delete(file)

        # Delete associated user-dataset relationships
        for user_dataset in dataset.users:
            print("Deleting User relationship")
            success, message = remove_dataset_from_user(user_dataset.UserUUID,user_dataset.DataSetUUID)
            print(success)
        dataset = session.query(DataSet).filter_by(UUID=dataset_uuid).one()
...
def delete_datasource(datasource_uuid):
    ...
    try:
        ...
        for location in datasource.locations:
            print(f"Deleting location: {location}")
            success, message = remove_location_from_datasource(datasource_uuid, location.Location, location.DateTime)
            print(success, message)

        # Delete associated sensors
        for sensor in datasource.sensors:
            print(f"Deleting sensor: {sensor.Sensor}")
            success, message = remove_sensor_from_datasource(datasource_uuid, sensor.Sensor)
            print(success, message)

        # Delete relationships with datasets
        for dataset in datasource.datasets:
            print(f"Removing dataset: {dataset.UUID} from datasource")
            success, message = remove_dataset_from_datasource(dataset.UUID)
            print(success, message)

        # Delete relationships with users
        for user_datasource in datasource.users:
            print(f"Deleting user-datasource relationship: {user_datasource}")
            success, message = remove_datasource_from_user(user_datasource.UserUUID, datasource_uuid)
            print(success, message)

        # Delete the datasource
        print(f"Deleting datasource: {datasource}")
